namespace FontAwesome5.Extensions
{
  ...
  public static class EFontAwesomeIconExtensions
  {
    ...
    /// </summary>
    public static bool GetSvg(this EFontAwesomeIcon icon, out string path, out int width, out int height, out string secondaryPath)
    {
      path = string.Empty;
      width = -1;
      height = -1;
      secondaryPath = string.Empty;
      if (FontAwesomeInternal.Information.TryGetValue(icon, out var info) && info.Svg != null)
      {
        path = info.Svg.Path;
        width = info.Svg.Width;
        height = info.Svg.Height;
        secondaryPath = info.Svg.SecondaryPath;
        return true;
      }
